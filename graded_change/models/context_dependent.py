...
# # Slower than scipy implementation, but much lower memory usage
...
#     for x_i in x:
#         for y_i in y:
#             distances.append(np.linalg.norm(x_i - y_i))
#     return distances

# Faster than scipy implementation, and much lower memory usage
@njit(nopython=True, parallel=True)
def dist_test(x,y):
    # len(x) by len(y)
    x_len = len(x)
    y_len = len(y)
    distances = np.zeros((x_len, y_len))
    for i in prange(x_len*y_len):
        distances[i//y_len, i%y_len] = np.linalg.norm(x[i//y_len]-y[i%y_len])
    return distances

def dist_func_wrapper(x,y):
    return np.mean(dist_test(x,y))
...
def compare_context_dependent_representations(dataset_dir, experiment_dir):
    ...
    for target in tqdm(targets):
        ...
        dist_dict = apply2dicts(c1_reps, c2_reps, dist_func_wrapper)
        dist = dict2array(dist_dict, [target])
        del c1_reps
        del c2_reps
        assert len(dist)==1
        dists.append(dist[0])
