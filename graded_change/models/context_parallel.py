...
def extract_representations_roberta_xlm(dataset_dir, experiment_dir, device="cpu", **kwargs):
...
# From https://stackoverflow.com/a/8290508/3558475
...
def save_representations(word_sents, model, tokenizer, device, rep_dir, max_sql=128):
    ...
    for word, sents in word_sents.items():
        ...
        for sent_batch in tqdm(batch_list(sents,batch_size), desc="Extracting Representations for '{}'".format(word),total=(len(sents)//batch_size+1)):
            try:
                word_hidden_states.extend(get_hidden_from_sent(sent_batch, model, tokenizer, device, max_sql, word))
...
def get_hidden_from_sent(sent_batch, model, tokenizer, device, max_sql,word):
    """ Returns BERT last layer activations with shape (layers, tokens, hidden_size) as well as the encoded sentence. """

    with torch.no_grad():
        expected = "<extra_id_0>"
        inputs = tokenizer([x.replace(word,'<mask>') for x in sent_batch],return_tensors="pt", max_length = 512, padding='max_length', truncation = True).to(device)
        outputs = model(**inputs,output_hidden_states=True)

        mask_indexes = [(inputs['input_ids']==tokenizer.mask_token_id).nonzero()[i][-1] for i in range(len(sent_batch))]
        # outputs.hidden_states[-1][0][mask_index]

        return [outputs.hidden_states[-1][i][mask_index].cpu().numpy() for i,mask_index in enumerate(mask_indexes)]
