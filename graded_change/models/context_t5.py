...
def extract_representations_t5(dataset_dir, experiment_dir, device="cpu", **kwargs):
    ...
    model = MT5ForConditionalGeneration.from_pretrained("models/t5_embedding_small").to(device)
    model.eval()
...
def get_hidden_from_sent(sent, model, tokenizer, device, max_sql,word):
    ...
    with torch.no_grad():
        expected = "<extra_id_0>"
        inputs = tokenizer(sent.replace(word,'<extra_id_0>'), pad_to_max_length = True, max_length = 256, truncation = True, return_tensors="pt").to(device)
        with tokenizer.as_target_tokenizer():
            labels = tokenizer(expected, return_tensors="pt").to(device)
        outputs = model(**inputs, labels=labels["input_ids"],output_hidden_states=True)

        return outputs.decoder_hidden_states[-1][0][0].cpu().numpy()
