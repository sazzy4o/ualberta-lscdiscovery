...
from models.context_t5 import extract_representations_t5
from models.context_bertin import extract_representations_bertin
from models.context_roberta_xlm import extract_representations_roberta_xlm
from models.context_roberta_xlm_v2 import finetune_roberta_xlm_v2, extract_representations_roberta_xlm_v2
...
parser.add_argument("--device", type=str, default="cuda:0", help="Name of device to train BERT model on: Usually one of {cpu, cuda}")
...
def predict(model_name="", dataset_dir="", overwrite=False, filtered=True, **params):
    ...
    elif model_name == "context-t5":

        print("Making classification dataset ...")
        make_classification_dataset(dataset_dir, experiment_dir)

        print("Extracting representations ...")
        extract_representations_t5(dataset_dir, experiment_dir, **params)

        print("Comparing context-dependent representations ...")
        compare_context_dependent_representations(dataset_dir, experiment_dir)
    elif model_name == "context-bertin":

        print("Making classification dataset ...")
        make_classification_dataset(dataset_dir, experiment_dir)

        print("Extracting representations ...")
        extract_representations_bertin(dataset_dir, experiment_dir, **params)

        print("Comparing context-dependent representations ...")
        compare_context_dependent_representations(dataset_dir, experiment_dir)
    elif model_name == "context-roberta-xlm":

        print("Making classification dataset ...")
        make_classification_dataset(dataset_dir, experiment_dir)

        print("Extracting representations ...")
        extract_representations_roberta_xlm(dataset_dir, experiment_dir, **params)

        print("Comparing context-dependent representations ...")
        compare_context_dependent_representations(dataset_dir, experiment_dir)
    elif model_name == "context-roberta-xlm-v2":

        print("Making classification dataset ...")
        make_classification_dataset(dataset_dir, experiment_dir)

        print("Finetuning roberta-xlm ...")
        finetune_roberta_xlm_v2(experiment_dir, **params)

        print("Extracting representations ...")
        extract_representations_roberta_xlm_v2(dataset_dir, experiment_dir, **params)
